    ...
    <div class="container">
        <div class="row">
            <div class="col-12">
                ...
                <form action="{{route('updated.video',$find_thisId->id)}}" method="post" style="width: 70%; margin:auto">
                    @csrf
                    @method('PUT')
                    <label for="video name">Video Name</label>
                    <input type="text" class="form-control" name="upvideo_name" placeholder="Set a name for this video" value="{{$find_thisId->utube_title }}">
                    <label for="video link">Link</label>
                    <input type="text" name="upnew_video" id="" class=" form-control" placeholder="Paste a Video link" value="{{$find_thisId->youtube }}">
                    <br>
